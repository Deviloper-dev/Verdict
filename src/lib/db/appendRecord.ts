 ...
 * Takes the per-group advisory lock (idempotent within a tx) to serialize
 * concurrent finalizations; UNIQUE(group_id, seq) is the backstop.
 */
export async function appendRecordTx(client: PoolClient, input: NewRecordInput): Promise<StoredRecord> {
